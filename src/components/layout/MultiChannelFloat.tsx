...
export function MultiChannelFloat() {
  ...
  return (
    <>
      {/* ── DESKTOP: Always-visible Email Pill ── */}
      <a
        href={`mailto:${RESORT_FACTS.email}`}
        className="hidden md:flex fixed bottom-[90px] right-8 z-50 items-center gap-2.5 group"
      >
        {/* Tooltip label */}
        <span className="bg-[var(--color-ivory)] text-[var(--color-forest)] text-xs px-3 py-1.5 rounded-lg shadow-md opacity-0 group-hover:opacity-100 transition-all duration-200 whitespace-nowrap font-medium tracking-wide">
          {RESORT_FACTS.email}
        </span>
        {/* Gold circle icon */}
        <div className="w-12 h-12 rounded-full bg-gradient-to-br from-[var(--color-gold)] to-[#8B6914] shadow-[0_4px_20px_rgba(194,162,78,0.6)] border-2 border-[var(--color-gold)] flex items-center justify-center text-white group-hover:scale-110 transition-all duration-300">
          <Mail size={20} strokeWidth={1.8} />
        </div>
      </a>

      {/* ── DESKTOP: Hover-to-expand Call + Book Now pill ── */}
      <div
        className="hidden md:flex fixed bottom-8 right-8 z-50 flex-col items-end space-y-3"
        onMouseEnter={() => setIsHovered(true)}
        onMouseLeave={() => setIsHovered(false)}
      >
        {/* Call — appears on hover */}
        <div className={clsx(
      ...
      <a
        href={`mailto:${RESORT_FACTS.email}`}
        className="md:hidden fixed bottom-[72px] right-4 z-50 flex items-center gap-2 bg-gradient-to-r from-[var(--color-gold)] to-[#8B6914] text-white px-4 py-2.5 rounded-full shadow-[0_4px_15px_rgba(194,162,78,0.5)] border border-[var(--color-gold)]/60"
      >
        <Mail size={16} />
